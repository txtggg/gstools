...
public final class ExcelWriter {
	 ...
	 * @param infos
	 */
	public static void setMarkers(InputStream in, boolean xls,List<Map<String,Object>> infos){
	...
	public static byte[] list2Excel(List<List<List<String>>> list, boolean xls) {
		...
		byte[] bytes = PoiUtil.workbook2ByteArray(wb);
		Files.closeIO(wb);
		return bytes;
		...
		if (rowKey != null) {
			...
			if (Lists.isEmpty(listInfo)) {// 如果为空,只需要把marker替换为空
				...
				for (int x = 1, s = listInfo.size(); x < s; x++) {
					...
					for (Entry<Integer, String> e : keyMap.entrySet()) {
						Row rowx = PoiUtil.insertRow(row.getSheet(), r + x);
						rowx.getCell(e.getKey()).setCellValue(Strings.toString(map.get(e.getValue())));
					}
